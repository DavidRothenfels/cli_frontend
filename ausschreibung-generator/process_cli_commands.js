        ...
        opencode.on('close', async (code) => {
            ...
            if (code !== 0) {
                ...
                if (output.trim()) {
                    ...
                    await createDocument(request_id, title, content, type)
...
async function createDocument(requestId, title, content, type) {
    try {
        ...
        if (response.ok) {
            ...
            await createLog(requestId, `✅ Dokument erstellt: ${title}`)
        }
    } catch (error) {
        console.error('❌ Failed to create document:', error)
        await createLog(requestId, `❌ Fehler beim Erstellen von ${title}: ${error.message}`, 'error')
    }
}
